...
if __name__ == '__main__':
  for split in SPLITS:
    ...
    for i in VIDEO_SETS[split]:
      ...
      instance_path = DATA_PATH + 'training/instance_02/{}.txt'.format(video_name)

      anns = open(ann_path, 'r')
      instances = open(instance_path, 'r')
      mask_lines = instances.readlines()
      ...
      for mask_line in mask_lines:
        mask_line = mask_line[:-1].split(' ')
        frame_id = int(mask_line[0])
        if frame_id not in mask_frames:
          mask_frames.update({frame_id: []})
        mask_frames[frame_id].append(mask_line)
      label_nums += len(mask_frames)
      ...
      for ann_ind, txt in enumerate(anns):
        ...
        ann_mask = None
        if frame_id in mask_frames:
          for mask_line in mask_frames[frame_id]:
            mask_cat = int(mask_line[2])
            mask_id = int(mask_line[1])%1000
            if int(mask_line[1])==10000:
              continue
            if mask_cat == cat_id and mask_id == track_id:
              ann_mask = {'size': [int(mask_line[3]), int(mask_line[4])], 'counts': mask_line[5]}
              #print(frame_id, cat_id, track_id)
              break
        #if ann_mask ==None and (cat_id == 1 or cat_id ==2):
